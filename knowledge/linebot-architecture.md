...
## Phase 5.1: ツバサ × DeepSeek V3.2 接続（2026-03-12）

### 変更内容
- `~/tsubasa/jan_client.py` を改修
- メインLLM: DeepSeek V3.2 API（deepseek-chat）
- フォールバック: JAN 4B（localhost:1337）※DeepSeek障害時のみ
- APIキーは `~/tsubasa/.env` に `DEEPSEEK_API_KEY` として格納
- バックアップ: `~/tsubasa/jan_client.py.bak.20260312`

### 技術構成
- `_call_llm()` 共通関数でDeepSeek/JAN両方を呼び分け
- DeepSeek: timeout 30秒、max_tokens 500
- JAN 4B: timeout 60秒（フォールバック時）
- 蒼真ツバサのシステムプロンプトは従来と同一

### 品質変化
- JAN 4B: 曖昧・短文・的外れな総評が頻発
- DeepSeek V3.2: 断定調・論理的・具体的アクションプラン提示
- 初回本番テスト（hsworking.com）: 100/100点 Aランク、総評品質◎

### コスト
- 1回の診断あたり約500-800トークン消費
- DeepSeek価格: 入力$0.28/1Mトークン + 出力$0.42/1Mトークン
- 1診断あたり約0.03円、月100診断でも約3円

### 次フェーズ
- Phase 5.2: エリカ × DeepSeek V3.2（AI相談・週次配信）
- Phase 5.3: マルモくん × DeepSeek V3.2（複合ニーズ対応）

## Phase 5.2: エリカ × DeepSeek V3.2 接続（2026-03-12）

### 変更内容
- `~/erika-line-bot/app.py` の `ask_llm()` 関数を改修
- メインLLM: DeepSeek V3.2 API（deepseek-chat）
- フォールバック: JAN 4B（localhost:1337）※DeepSeek障害時のみ
- APIキーは `~/erika-line-bot/.env` に `DEEPSEEK_API_KEY` として格納
- バックアップ: `~/erika-line-bot/app.py.bak.20260312`

### 技術構成
- RAG検索（ChromaDB + HuggingFace embeddings）は従来どおり維持
- RAGで取得したcontextをDeepSeek V3.2に渡してエリカが回答
- DeepSeek: timeout 30秒、max_tokens 500
- JAN 4B: timeout 60秒（フォールバック時）

### 期待効果
- JAN 4B: 200文字制限でも的外れ・情報不足が頻発
- DeepSeek V3.2: RAG情報を正確に活用し、具体的なURL付き案内が可能に
- AI導入相談の品質向上 → コンサル契約転換率の向上

### コスト
- 1回の会話あたり約300-600トークン消費
- 月100会話でも約2-5円

## Phase 5.3: マルモくん × DeepSeek V3.2 接続（2026-03-12）

### 変更内容
- `~/hs_a2a/jan_client.py` を改修
- メインLLM: DeepSeek V3.2 API（deepseek-chat）
- フォールバック: JAN 4B（localhost:1337）※DeepSeek障害時のみ
- APIキーは `~/hs_a2a/.env` に `DEEPSEEK_API_KEY` として格納
- バックアップ: `~/hs_a2a/jan_client.py.bak.20260312`
- 関数名 `ask_jan()` は互換性のため維持（内部でDeepSeek優先呼び出し）

### テスト結果（3問）
- 「3人で使いたい」→ 個室ブース提案 + 料金 + URL（5.0秒）
- 「プロジェクター」→ 会議室案内 + モニター貸出言及（3.7秒）
- 「ランチ」→ 周辺飲食店案内 + スタッフ連携（4.0秒）
- 全問DeepSeek V3.2で応答、JAN 4Bフォールバック未使用

### 追加修正: push_to_admin停止（2026-03-12）
- line_bot_server.py L715-716: 手動モード時のpush_to_admin をコメントアウト
- L717: log_message() によるログ記録に切替
- 目的: LINE無料メッセージ枠の消費防止

## Phase 5 完了サマリー（2026-03-12）

### 全AIスタッフ DeepSeek V3.2 接続完了
| スタッフ | ファイル | メインLLM | フォールバック | 応答時間 |
|---------|---------|----------|-------------|---------|
| ツバサ | ~/tsubasa/jan_client.py | DeepSeek V3.2 | JAN 4B | 3-5秒 |
| エリカ | ~/erika-line-bot/app.py | DeepSeek V3.2 | JAN 4B | 3-5秒 |
| マルモくん | ~/hs_a2a/jan_client.py | DeepSeek V3.2 | JAN 4B | 3-5秒 |

### 月間コスト見積もり
- ツバサ: 月100診断 × 800トークン = 約3円
- エリカ: 月100会話 × 600トークン = 約3円
- マルモくん: 月300会話 × 350トークン = 約5円
- 合計: 約11円/月（5Mトークン無料枠で数ヶ月カバー）

### 次フェーズ
- Phase 6: DeerFlowサブエージェント統合（ツバサ競合分析・エリカリサーチ）
- Phase 7: バーチャルオフィス会員向けSlack/Telegramチャネル提供

## Phase 6: ツバサ競合分析機能（2026-03-13）

### 新規ファイル
- `~/tsubasa/competitor_analysis.py` — DeepSeek V3.2直接呼び出しで競合分析レポート生成

### 機能
- `analyze_competitors(diagnosis_result)`: 単体サイト診断結果から強み/弱み/改善コード/アクションプラン生成
- `compare_sites(target_url, competitor_urls)`: 複数サイト同時診断→比較レポート生成
- `run_multi_diagnosis(urls)`: 複数URL一括診断

### LINE Bot統合（app.py改修）
- 診断後に「詳細分析」「競合分析」「詳細」と送信で詳細レポートをpush
- `last_diagnosis_results` 辞書でユーザーごとの最新診断結果をキャッシュ
- `run_analysis_async()` で非同期実行、LINE 5000文字制限対応（自動分割）

### レポート内容（3セクション構成）
1. 競合比較分析（強み3つ・弱み3つ）
2. 即実装可能な改善コード（HTMLスニペット付き）
3. 今日・今週・今月のアクションプラン

### DeerFlow 2.0 について
- Docker稼働中（localhost:2026）だがTavily/Jina APIキー未設定のためWeb検索機能は未稼働
- LangGraph API経由のcontext方式での呼び出しは確認済み
- 将来Tavily無料キー取得後にフルエージェント機能を有効化予定

### エリカ週次配信 DeepSeek V3.2化
- `~/erika-line-bot/ai_trend_broadcast.py` 改修済み
- メインLLM: DeepSeek V3.2、フォールバック: JAN 4B
- テスト生成: vol.4（485文字）品質確認済み

## Phase 6.2: ツバサ定期監査レポート（2026-03-13）

### 新規ファイル
- `~/tsubasa/tsubasa_audit.py` — 週次/月次SEO/AIO/LLMO/GEO/A2A監査

### スケジュール（LaunchAgent）
- 週次: 毎週月曜 7:30（com.hsbuilding.tsubasa-audit-weekly）
- 月次: 毎月1日 7:00（com.hsbuilding.tsubasa-audit-monthly）

### 対象ページ（10ページ）
トップ, コワーキング, 個室ブース, 会議室, 料金, アクセス, AI対策, エリカ, ツバサ, バーチャルオフィス

### レポート内容
- 週次: スコア一覧 + 前週比較（↑↓→）+ 0点項目ハイライト
- 月次: 週次サマリー + 5軸分析（SEO/AIO/LLMO/GEO/A2A）+ アクションプラン + 戦略提言

### 初回監査結果（2026-03-13）
- 8/10ページがAランク（100点）
- 要改善: /ai-solution(404), /price(h1なし), /coworking(h1重複)

### 配信先
- 管理者（ADMIN_USER_ID）のみにpush送信
- 無料メッセージ消費: 週1+月1 = 月5通程度

## Phase 7: Search Console API連携 ブログ記事テーマ自動提案（2026-03-13）

### 新規ファイル
- ~/tsubasa/gsc_blog_suggester.py – GSCデータ取得→キーワード分析→DeepSeek V3.2でブログテーマ生成→LINE push
- ~/tsubasa/gsc-service-account.json – Google Cloud サービスアカウントキー

### Search Console API設定
- プロジェクト: My First Project (total-messenger-447708-c8)
- サービスアカウント: tsubasa-seo@total-messenger-447708-c8.iam.gserviceaccount.com
- 権限: Search Console「制限付き」ユーザー
- 対象サイト: https://www.hsworking.com/

### 分析ロジック（3層）
- 高インプレッション×低CTR: imp≧50 かつ CTR<15% → 記事改善 or 新規記事チャンス
- 2ページ目圏内: 順位11-30 かつ imp≧20 → もう少しで上位表示
- ブランドKW: hsビル/エリカ/ツバサ/マルモ含むクエリ → ブランド認知トラッキング

### DeepSeek出力フォーマット（記事テーマ3案）
- タイトル案（32文字以内）
- 狙うキーワード
- 想定読者・成約導線
- H2構成（4-6個）
- JSON-LD設計（Article + 推奨スキーマ）
- llm.txt追記案
- FAQPage候補（2-3問）
- 内部リンク先

### LaunchAgent
- com.hsbuilding.tsubasa-blog-suggest: 毎週月曜7:45
- ログ: ~/tsubasa/logs/blog_suggest/

### 週次配信スケジュール（月曜朝）
| 時刻 | LaunchAgent | 内容 |
|------|------------|------|
| 7:30 | com.hsbuilding.tsubasa-audit-weekly | SEO/AIO週次監査 |
| 7:45 | com.hsbuilding.tsubasa-blog-suggest | ブログ記事テーマ提案 |
| 8:00 | com.hsbuilding.ai-trend | エリカAIニュース配信 |
| 9:00 | com.hsbuilding.weekly-report | マルモ/エリカ週次レポート |

### 初回テスト結果（2026-03-13）
- 取得クエリ: 50件（28日間）
- 改善チャンスKW: 多数（claude/gemini比較系がCTR7-13%）
- 主要発見: 「コワーキングスペース 奈良」CTR43.1%/順位3.8（コア事業KW強い）
- 提案テーマ例: 「奈良でWeb会議できる個室7選」→個室ブース予約導線

## Phase 8: AIエージェント自動発見インフラ（2026-03-13）

### 新規エンドポイント（Wix http-functions.js）
- `GET /_functions/ai_plugin` → ai-plugin.json（ChatGPT/Copilot等の自動発見用）
- `GET /_functions/openapi_json` → OpenAPI 3.1.0仕様書（6エンドポイント定義）

### Custom GPT（GPT Store）
- 名前: HSビル予約エージェント｜奈良コワーキング
- URL: https://chatgpt.com/g/g-69b390b68c108191a494e00b721fed02-hshiruyu-yue-esiento-nai-liang-kowakinku
- 公開範囲: GPTストア（全世界公開・ドメイン認証済み hsworking.com）
- Actions: OpenAPI 3.1.0経由で6エンドポイント接続済み
- テスト結果: getCatalog→4サービス取得OK、getBookingLink→予約URL+クーポン生成OK

### llm.txt
- v2.7.0 → v2.8.0（2026-03-13更新）
- 追記: ai_plugin, openapi_json エンドポイント、AI Agent Discoveryセクション、Custom GPT URL

### 5層AIエージェント発見インフラ（全ライブ）
| 層 | URL | 役割 |
|----|-----|------|
| llm.txt v2.8.0 | https://www.hsworking.com/llm.txt | AIが読む施設ガイド |
| JSON-LD | コワーキングページ埋込 | Google/AI Overviews |
| A2A Protocol | 12エンドポイント | 機械間対話 |
| ai-plugin.json | /_functions/ai_plugin | ChatGPT等の自動発見入口 |
| OpenAPI 3.1.0 | /_functions/openapi_json | API設計図 |
| Custom GPT | chatgpt.com/g/... | ChatGPTユーザー向け予約エージェント |

---

## Phase 8: VO郵便転送対応 Layer 0.5（2026-03-19）

### 概要
バーチャルオフィス契約者向けの郵便転送問い合わせにマルモが自動対応する仕組みを実装。
VO契約者をLINE user_idで識別し、プラン別の案内を自動で付与する。

### 処理階層（更新後）
Layer 0: FAQ即答（239パターン ← 221+18カテゴリ追加） Layer 0.5【新設】: VO契約者判定 → 郵便転送専用フロー Layer 1: 予約フロー（Wix API） Layer 1A: booking_link_helper（予約リンク生成） Layer 2: プラン候補提示 Layer 3: DeepSeek V3.2推論 Layer 4: スタッフ引き継ぎ


### 新規ファイル
| ファイル | パス | 役割 |
|---------|------|------|
| vo_handler.py | ~/hs_a2a/vo_handler.py | VO契約者判定・郵便対応・管理コマンド |
| vo_members.json | ~/hs_a2a/vo_members.json | VO契約者リスト（user_id管理） |
| vo_pending.json | ~/hs_a2a/vo_pending.json | 承認待ちキュー（自動生成） |

### FAQ追加（18カテゴリ / 106トリガー）
- 転送基本・料金・依頼・急ぎ・停止変更
- 受取可（ポスト投函型）・受取可（サイン必要型）・宅配便・受取不可
- 来店受取・身分証・DM廃棄・宛名・保管期限・不在票
- 届いてるか確認・ミニマムプラン・プラン比較

### 受取ジャンル3分類
- ✅ 確実に受取（ポスト投函型）: 普通郵便/速達/レターパックライト/メール便/ネコポス
- ⚠️ 受取可能だが不在時は不在票: 簡易書留/一般書留/レターパックプラス/宅配便
- ❌ 受取不可: 現金書留/代引き/本人限定受取/大型荷物/危険物/生鮮/クール便

### VO契約者登録フロー
- A方式（自己申告）: ユーザーが「バーチャルオフィス契約しました」→ マルモが名前・プランをヒアリング → vo_pending.jsonに保存 → 管理者が承認 → vo_members.jsonに自動登録
- B方式（管理コマンド）: 管理者がLINEで直接登録
- C方式（将来）: Wix会員APIとの自動同期

### 管理コマンド（管理者LINE限定）
| コマンド | 機能 |
|---------|------|
| hs:vo承認待ち | 承認待ち申請の一覧表示 |
| hs:vo承認 番号 | 指定番号の申請を承認→会員登録 |
| hs:vo登録 ユーザー名 | 最新の承認待ちを即登録 |
| hs:vo一覧 | 登録済みVO会員の一覧 |

### line_bot_server.py 変更箇所
- import vo_handler 追加（6行目）
- Layer 0.5 挿入（737行目付近、faq_matcher.matchの直前）
- VO管理コマンド挿入（634行目付近、管理者コマンドブロック内）
